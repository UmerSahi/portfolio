...
export interface ScrollStackProps {
  ...
  scrollerRefOut?: React.RefObject<HTMLDivElement | null>;
}
...
export const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const scrollerRef = scrollerRefOut || localScrollerRef;
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (useWindowScroll) {
        ...
        // Container scroll mode: cards pin upon reaching stack position and STAY securely in the stack
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // In window scroll mode, window scroll event works directly with global scroll
      ...
      const lenis = new Lenis({
        wrapper: scroller,
        content: innerEl as HTMLElement,
        duration: 1.2,
      ...
      // Boundary wheel listener: if user wheels beyond top/bottom of container, pass scroll to window
      const handleBoundaryWheel = (e: WheelEvent) => {
        const atTop = scroller.scrollTop <= 2 && e.deltaY < 0;
        const atBottom =
      ...
      scroller.addEventListener("wheel", handleBoundaryWheel, { passive: true });
